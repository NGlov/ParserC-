...
map<string, Token> mapofTok; //token map 
map<string, bool> mapOfVar; //var map
map<string, Value> mapOfVal; //val map
queue <Value>* ListOfVal; //vallist map
...
bool PrintStmt(istream& in, int& line) {
    ...
    while (!ListOfVal->empty()) {
        cout << ListOfVal->front();
        ListOfVal->pop();
    }
    cout<< "\n";
    return ex;
...
bool VarList(istream& in, int& line, LexItem& idtok, int strlen) {
    ...
    if (tok == IDENT)
    {
        ...
        if (mapOfVar.find(identstr) != mapOfVar.end()) {
            ParseError(line, "Variable Redefinition");
            return false;
        }
        mapOfVar[identstr] = false;

        if (idtok == CHARACTER) {
            mapOfVar[identstr] = true;
            string str(strlen, ' ');

            Value ret(str);

            mapOfVal[identstr] = ret;
            mapofTok[identstr] = idtok.GetToken();
    ...
    if (tok == ASSOP)
    {
        ...
        if (idtok == CHARACTER) {
            ...
            if (ret.IsInt()) {
                ...
                mapOfVal[identstr] = Value(newReal);
            }
        }
        else {
            mapOfVal[identstr] = ret;
...
bool AssignStmt(istream& in, int& line) {
    ...
    if (varstatus) {
        ...
        if (t == ASSOP) {
            ...
            mapOfVar[h.GetLexeme()] = true;
            if (mapofTok[h.GetLexeme()] == CHARACTER) {
                if (r.IsString() == false) {
                    ParseError(line = line -2, "Illegal mixed-mode assignment operation");
                    return false;
                }

                int strlen = mapOfVal[h.GetLexeme()].GetString().length();
...
bool Factor(istream& in, int& line, int sign, Value& retVal) {
    ...
    if (tok == IDENT) {
        ...
        if (!(mapOfVar.find(lexeme)->second))
        {
            ParseError(line, "Using Undefined Variable");
            return false;
        }

        retVal = mapOfVal[lexeme];


        if (sign == 1 || sign == -1) {
            if (mapOfVal[lexeme].IsInt()) {
                retVal = Value(sign * (mapOfVal[lexeme].GetInt()));
            }
            if (mapOfVal[lexeme].IsReal()) {
                retVal = Value(sign * (mapOfVal[lexeme].GetReal()));
            }
            if (mapOfVal[lexeme].IsString()) {
